...
// إضافة واجهة لتخزين الصور المرفوعة
...
export interface JacketState {
  ...
  totalPrice: number;
  isCapturing: boolean;
  uploadedImages: UploadedImage[]; // إضافة مصفوفة الصور المرفوعة
}
...
export interface JacketContextType {
  ...
  // إضافة دوال إدارة الصور المرفوعة
...
const initialState: JacketState = {
  ...
  totalPrice: 220,
  isCapturing: false,
  uploadedImages: [], // تهيئة مصفوفة الصور المرفوعة
};
...
export const JacketProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  // تأخير حفظ البيانات لتجنب الحلقة اللا نهائية
  useEffect(() => {
    if (!isInitialized) {
