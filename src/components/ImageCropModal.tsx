...
const ImageCropModal: React.FC<ImageCropModalProps> = ({
  ...
  const handleApplyCrop = async () => {
    ...
    try {
      ...
      onCropComplete(croppedUrl, imageFile);
      setIsProcessing(false);
      onClose();
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
    } catch (e) {
      setIsProcessing(false);
      alert("حدث خطأ أثناء الاقتطاع");
    }
  };

  const handleCropModeChange = (mode: "free" | "square" | "circle") => {
    setCropMode(mode);
    setCrop(undefined); // سيحدد crop جديد عند تحميل الصورة
    setCompletedCrop(undefined);
  };

  // ==================
  // هنا يبدأ JSX للنافذة
  // ==================

  // استخدم React Portal هنا
        ...
        >
          ...
          >
            ...
            >
              {/* Main Crop Area */}
              <div
                className={`flex-1 overflow-auto ${isMobile ? "p-2" : "p-6"}`}
              >
